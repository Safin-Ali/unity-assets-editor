...
import { ZipDeserializer } from "./classes/ZipHandler.ts";
...
const inputPath = pathGen(`assets`,"simple.zip");

const fileIns = new FileHandler({
  inputPath,
  outPath:pathGen("output","simple_damaged.zip")
})

const zipDesIns = new ZipDeserializer(fileIns.buffer!);