...
export class TSPHandler {
  ...
  #parseAssetStructure(buffer) {
    try {
      ...
      this.#fillDependencyObjects(buffer, hexHandler);
   ...
   * @param {HexHandler} hexHandler - Instance of HexHandler to manage hex data.
   */
  #fillDependencyObjects(buffer, hexHandler) {
  ...
  #addSpawnType (buffer) {
    ...
    let fileIdBuf = intToHexBytes(this.#assetStructure.dependencyObjects.length)
    ...
    const newDepFileId = [...fileIdBuf,"01","00","00","00",...new Array(4).fill("00")];    

    let startOffset = this.#assetStructure.sometimesSpawn.offset;
    
    if(this.#tspPromptValues.spawnType) {
      // modify sometimes length value
      const newSometimesLeng = intToHexBytes((hexToInt(this.#assetStructure.sometimesSpawn.lengthValue)+this.#monoAssetTextBuffer.length)) 
      hexHandler.replaceBytes(this.#assetStructure.sometimesSpawn.lengthOffset,newSometimesLeng);
