...
parentPort.on("message", (baseAssetsArg) => {
    baseAssets = baseAssetsArg;
...
const manipulateObj = (indexStr) => {
    try {
        ...
        const hexIns = new HexHandler(fileIns.buffer);
        const monoDepOffset = hexIns.findIndex(asciiToHexBytes(baseAssets.mono));
        const skinDepOffset = hexIns.findIndex(asciiToHexBytes(baseAssets.skin));
        const monoAliasOffset = hexIns.findIndex(asciiToHexBytes(busHD_01Alias));
...
const manipulateSkin = (indexStr) => {
    try {
        ...
        const hexIns = new HexHandler(fileIns.buffer);
        const skinAliasOffset = hexIns.findIndex(asciiToHexBytes(baseAssets.skinAlias));
